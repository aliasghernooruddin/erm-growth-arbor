<template>
  <section>
    ...
    <v-dialog v-model="isApprove" width="400">
      <v-card>
        <v-card-title> Forward to CEO </v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
...
<script>
import CommitteeAPIs from "@/service/apis/committee.js";
...
export default {
  ...
  computed: {
    ...mapGetters(["USERINFO"]),
    show() {
      if (this.risk["level"] == "Risk Committee") return true;
      else return false;
    },
  },
  methods: {
    getRisks() {
      ...
      CommitteeAPIs.getRisks(id).then((res) => {
        if (res["status"]) {
          this.risks = res["data"];
        }
      });
    ...
    submit(type) {
      ...
      CommitteeAPIs.updateRisk(data).then((res) => {
        if (res["status"]) {
          this.comments = "";
